...
Jej książka "The deficit myth", opublikowana w 2020 r., stała się bestsellerem New York Timesa i zajmowała 3 miejsce na liście bestsellerów Amazona. Wciąż jest bardzo popularna i obecnie jest na [6 miejscu](https://www.amazon.com/gp/bestsellers/books/2599/ref=pd_zg_hrsr_books) w kategorii Public Finance (12 w kategorii Money & Monetary Police i 13 w kategorii Economic Policy & Development).
...
2.  W sytuacji recesji wytworzenie dodatkowego dochodu jest możliwe dzięki wyższym wydatkom, gdyż powodują one przede wszystkim wzrost rozmiarów produkcji.
    ...
    Wynika to z istnienia niewykorzystanych mocy produkcyjnych w tym okresie.
...
[^geneza-7]: Powstaje pętla relacji: wzrost cen powoduje wzrost płac nominalnych, które wpływają na wzrost cen itd.
...
Nowoczesna teoria monetarna odrzuca w ten sposób charakterystyczne dla teorii neoklasycznej założenia dotyczące na przykład reprezentatywnego podmiotu dążącego do maksymalizacji zysku[^geneza-8], dążenia systemu do równowagi w długim okresie[^geneza-9], gdzie problem efektywnego popytu zanika jako krótkookresowy, a także doskonałej elastyczności płac i cen[^geneza-10] przy substytucji między pracą a kapitałem[^geneza-11], określanej relacją cen odzwierciedlających z kolei relatywną rzadkość danego zasobu -- co jest kluczowym mechanizmem prowadzącym gospodarkę do stanu równowagi przy pełnym zatrudnien
...
[^geneza-8]: Czyli "przeciętnego Polaka-cebulaka", który jest zainteresowany jedynie jak największym zyskiem.

[^geneza-9]: Inaczej mówiąc, jeśli poczekamy wystarczająco długo, to w końcu zrobi sie normalnie.

[^geneza-10]: Płace i ceny natychmiast dostosowują się do zmian podaży i popytu.

[^geneza-11]: Co oznacza, że można swobodnie zastępować pracę ludzką maszynami i odwrotnie.
...
6.  Paradoks płynności[^geneza-12] (sformułowany przez S. Dow i A. Nesvetailovą) -- nowe sposoby kreacji płynności kończą się transformacją płynnych aktywów w niepłynne;
...
[^geneza-12]: płynność aktywów to zdolność do szybkiej, łatwej i bez nadmiernej straty zamiany papierów wartościowych na środki pieniężne.
...
W świecie knightowskiej niepewności[^geneza-13] i asymetrii informacji, ciąg kolejnych lat, w których firmy realizują lub nawet przekraczają plany sprzedażowe powoduje, że skłonność do podejmowania ryzyka rośne.
...
[^geneza-13]: Czyli takiej, w której nie da się statystycznie oszacować prawdopodobieństwa wystąpienia przyszłych zdarzeń.
...
1.  Planowanie produkcji.
    ...
    Po zaplanowaniu produkcji firmy mogą ustalić zapotrzebowanie na pracowników[^geneza-14] a zatem również koszty.
    Znając koszty, przedsiębiorstwa ustalają swoje ceny.
    Ceny są kalkulowane jako koszty plus marża[^geneza-15]
...
3.  Konsumpcja.
    ...
    Skłonność do oszczędzania gospodarstw domowych zależy od poziomu dochodów -- im są wyższe, tym mniejszy jest udział wydatków w dochodach[^geneza-16].
...
4.  Zwrot pieniądza.
    ...
    Niemniej część oszczędności pozostaje w sektorze bankowym przez co zwiększa się zadłużenie przedsiębiorstw i zmniejsza ich chęć do inwestowania[^geneza-17].

[^geneza-14]: Ponieważ półprodukty są wytwarzane przez pracowników i ich składniki oraz składniki składników też są wytwarzane przez pracowników to ostatecznie wszystkie koszty są kosztami pracy.

[^geneza-15]: Szerzej na ten temat pisze (Lavoie, 2014).

[^geneza-16]: W ten sposób duże nierówności dochodowe utrudniają firmom „odzyskanie" środków zainwestowanych w produkcję.

[^geneza-17]: Problem ten nie występuje, jeśli pieniądz służy tylko jako jednostka miary i nie jest postrzegany jako medium wymiany czy nośnik wartości.
...
Jeśli zdecydują się mniej konsumować a więcej oszczędzać (np. na przyszłe emerytury lub spodziewając się bezrobocia) to spadnie sprzedaż i wzrośnie zadłużenie firm[^geneza-18].
Oba te czynniki działają w kierunku obniżenia produkcji w gospodarce.

[^geneza-18]: Zadłużenie firm nie wzrośnie proporcjonalnie, gdyż jak wspomniano, mogą „przejąć" część oszczędności emitując np. akcje.
