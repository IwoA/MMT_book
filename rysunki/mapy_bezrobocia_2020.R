...
powiaty_mapa <- left_join(powiaty_df,
                          select(bezrobocie_2020, Teryt, Wartosc),
                          by = c("id" = "Teryt"))
...
powiaty_mapa2 <- left_join(powiaty_df,
                          select(bezrobotni_2020, Teryt, Wartosc),
                          by = c("id" = "Teryt"))
...
beeswarm(bezrobocie_2020$Wartosc, method = "center", pch = 16, ylab = "Stopa bezrobocia w %")
abline(h=median(bezrobocie_2020$Wartosc), col = "red")
abline(h=quantile(bezrobocie_2020$Wartosc, probs = .75), col = "blue", lty = 2)
text(x = 0.6, y = median(bezrobocie_2020$Wartosc)+0.8, "50%")
text(x = 0.6, y = 11, "75%")

par(mfrow = c(1,1), mar=rep(3,4), pin = c(4,5))
beeswarm(bezrobotni_2020$Wartosc/1000, method = "center", pch = 16, ylab = "Liczba bezrobotnych w tys.")
abline(h=median(bezrobotni_2020$Wartosc/1000), col = "red")
abline(h=quantile(bezrobotni_2020$Wartosc/1000, probs = .75), col = "blue", lty = 2)
text(x = 0.55, y = median(bezrobotni_2020$Wartosc/1000)+0.8, "50%")
text(x = 0.55, y = quantile(bezrobotni_2020$Wartosc/1000, probs = .75)+0.8, "75%")
...
bezrobotni_miesiace <- read_delim("RYNE_2961_CREL_20200909bezrobotni_miesiace.csv", 
                                 ";", escape_double = FALSE, 
                                  col_types = cols(Atrybut = col_skip(),
                                                   Płeć = col_skip(),
                                                   `Jednostka miary` = col_skip()),                                  
                                  trim_ws = TRUE)
...
top <- bezr %>% filter(data =="2020 II kw.") %>% top_n(Wartosc,n = 10)
bottom <- bezr %>% filter(data =="2020 II kw.") %>% top_n(Wartosc,n = -10) %>% arrange(Wartosc)
...
ggplot(top10,aes(x= data, y = Wartosc, group=Nazwa)) + 
        labs(x = "", y = "Stopa bezrobocia") +
        geom_line() + facet_wrap(vars(Nazwa), nrow = 2) + 
        theme_minimal() + 
        scale_x_discrete(breaks=c("2018 I kw.","2020 II kw.")) +
...
bot10$Nazwa <- factor(bot10$Nazwa, levels = deframe(kolejnosc1))
